...
void PoolingLayer::SetUp(const std::vector<Blob*> &bottom, std::vector<Blob*> *top) {
...
void PoolingLayer::Forward(const std::vector<Blob*> &bottom, std::vector<Blob*> *top) {
                                ...
                                for (int c = 0; c < CHANNELS_; ++c) {
                                        for (int ph = 0; ph < POOLED_HEIGHT_; ++ph) {
                                                for (int pw = 0; pw < POOLED_WIDTH_; ++pw) {
                                                        ...
                                                        int hend = std::min(hstart + KSIZE_, HEIGHT_);
                                                        int wend = std::min(wstart + KSIZE_, WIDTH_);
...
float PoolingLayer::Backward(const std::vector<Blob*> &top, const bool propagate_down, std::vector<Blob*> *bottom) {
        if (!propagate_down) {
            return 0.;
        }
        ...
        // Different pooling methods. We explicitly do the switch outside the for
        // loop to save time, although this results in more codes.
        memset(bottom_diff, 0, (*bottom)[0]->count() * sizeof(float));
        switch (this->layer_param_.pool()) {
                case LayerParameter_PoolMethod_MAX:
                // The main loop
